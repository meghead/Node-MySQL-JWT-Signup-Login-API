...
function makeDb( config ) {
 ...
 var addNewUserTest = async function(req,res, next){
 	var date = new Date();
	var hashedpassword = md5(req.body.password);
    var name = req.body.name,
    var email = req.body.email,
	var gavatar = req.body.gavatar, 
...
try {	
	...
	if(row_a == 0) {
		row_b = await db.query( 'INSERT INTO  ?? SET  (name,email,gavatar,password,firetoken) VALUES (?,?,?,?,?)', ['accounts', name, email, gavatar, password,fireToken]);	
        if(row_b > 0) {
			const ugavatar  = row_b[0].insertId + '-user.jpg';
			const userid = row_b[0].insertId;
